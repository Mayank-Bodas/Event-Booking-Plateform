...
const NavBar = () => {
  ...
  const { isOrganizer, isStaff, isAdmin } = useRoles();
      ...
      <div className="container mx-auto p-4">
        <div className="flex justify-between items-center">
          <div className="flex gap-10 md:gap-20 items-center">
            <Link to={isOrganizer ? "/organizers" : "/"}>
              <h1 className="text-xl font-bold">Event Ticket Platform</h1>
            </Link>
            <div className="text-gray-300 flex gap-8">
              {isOrganizer && <Link to="/dashboard">Dashboard</Link>}
              {isOrganizer && <Link to="/dashboard/events">Events</Link>}
              {isOrganizer && <Link to="/dashboard/ticket-sales">Ticket Sales</Link>}
              {isOrganizer && <Link to="/dashboard/ticket-types">Ticket Types</Link>}
              {isOrganizer && <Link to="/dashboard/reports">Reports</Link>}
